...
postgres_engine = create_async_engine(
    ...
    connect_args={
        ...
        "timeout": 5,          # asyncpg connect timeout (seconds)
        "command_timeout": 8,  # asyncpg per-query timeout (seconds)
    }
)
...
async def init_db():
    ...
    # Quick connectivity check — only attempt DDL if the connection succeeds
    try:
        async with timescale_engine.begin() as conn:
            ...
            # Try TimescaleDB hypertable creation — skip silently on standard Postgres (Supabase)
            for table, query in [
                ...
                try:
                    ...
                    logger.info(f"Hypertable {table} ready.")
                except Exception as ts_err:
                    logger.info(f"Skipping TimescaleDB hypertable {table} (not supported on this DB): {ts_err}")
